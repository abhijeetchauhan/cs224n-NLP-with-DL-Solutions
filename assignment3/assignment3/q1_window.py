...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    windowed_data = []
    initdata = [start for i in xrange(window_size)]
    enddata = [end for i in xrange(window_size)]
    conf = Config()
    for sentence, labels in data:
    ### YOUR CODE HERE (5-20 lines)
        sentence = initdata + sentence + enddata
        sentence = [feature for word in sentence for feature in word]
        temp = [(sentence[i*conf.n_word_features:(i+2*window_size+1)*conf.n_word_features],labels[i]) for i in xrange(len(labels))]
        windowed_data = windowed_data + temp
    ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        conf = Config()
        self.input_placeholder = tf.placeholder(tf.int32,(None, conf.n_window_features))
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings, (-1,conf.n_window_features*conf.embed_size))                                                                                                         
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        conf = Config()
        W = tf.get_variable("W",shape=[conf.n_window_features*conf.embed_size, conf.hidden_size],
           initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable("U",shape=[conf.hidden_size, conf.n_classes],
           initializer=tf.contrib.layers.xavier_initializer())        
        b1 = tf.get_variable("b1",shape=conf.hidden_size,initializer=tf.zeros_initializer())
        b2 = tf.get_variable("b2",shape=conf.n_classes,initializer=tf.zeros_initializer())
